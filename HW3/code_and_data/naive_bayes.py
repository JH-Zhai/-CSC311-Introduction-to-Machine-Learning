...
def train_mle_estimator(train_images, train_labels):
    ...
    t_s = np.sum(train_labels, axis=0)
    theta_mle = (train_images.T).dot(train_labels)/t_s
    pi_mle = t_s/train_labels.shape[0]
...
def train_map_estimator(train_images, train_labels):
    ...
    # YOU NEED TO WRITE THIS PART
    t_s = np.sum(train_labels, axis=0)
    theta_map = ((train_images.T).dot(train_labels) + 2) / (t_s + 4)
    pi_map = t_s / train_labels.shape[0]
...
def log_likelihood(images, theta, pi):
    ...
    # YOU NEED TO WRITE THIS PART
    f = np.log(pi)+np.dot(images, np.log(theta))+np.dot((1-images), np.log(1-theta))
    # note to import e from math module
    p = np.array(e)**f
    s = np.log((np.sum(p, axis=1)).reshape(f.shape[0], 1))
    log_like = f - s
    return log_like
...
def accuracy(log_like, labels):
    ...
    acc = np.mean(pred == labels.argmax(axis=1))
    return acc
