...
def knn(k, train_data, train_labels, valid_data):
    ...
    valid_labels = (np.mean(valid_labels, axis=1) >= 0.5).astype(int)#np.int???????????
...
def run_knn():
    ...
    for i in k:
        ...
        t_count = 0
        for j in range(valid_targets.size):
            if valid_targets[j] == valid_labels[j]:
                t_count += 1
        rate.append(t_count / valid_targets.size)
    plt.plot(k,rate)
    plt.xlabel("k")
    plt.ylabel("classification rate on validation")
    plt.show()
